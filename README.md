...
Results example output could be seen in file `src/exp/interpolation_sanity_check/results.ipynb`

to run experiment:

```bash
venv_vis/bin/python src/exp/interpolation_sanity_check/run.py {path/to/config}
```

instead of `venv_vis/bin/python` you can use just `python`, if venv_vis environment already activated.


`path/to/config` is optional, you can specify your own, or not, then `src/configs/interpolation_sanity_check_base.yaml` will be used.
...
Results example output could be seen in file `src/exp/separation_spot/results.ipynb`

to run experiment:

```bash
python src/exp/separation_spot/run.py {src/configs/separation_spot_base.yaml}
```

you can specify your `path/to/config`, or not, then `src/configs/separation_spot_base.yaml` will be used.