...
def main(mode, args):
    ...
    ys = []
    assert args.num_sampling_steps % (batch_size) == 0
    step_size = args.num_sampling_steps // (batch_size)

    for i in range(batch_size):
        y = [[class_a] * (batch_size - i) + [class_b] * (i + 1) for _ in range(step_size)]
        ys.extend(y)
    ...
    dirname = f'image_{class_a}_{class_b}_{args.num_sampling_steps}_{batch_size}'
    os.makedirs(dirname, exist_ok=True)
    for i, sample in enumerate(samples):
        save_image(sample, f"{dirname}/{i * step_size}.png", normalize=True, value_range=(-1, 1))
